  ...
  const statusLabel = state.status === "paused"
    ...
    : state.status === "finished"
      ...
      : state.status === "error"
        ? labels.failed
        : labels.reading;
  return <section aria-label={labels.reading} aria-live="polite" onKeyDown={onSectionKeyDown} className="flex min-h-12 items-center gap-2 overflow-x-auto border-b border-border-light bg-bg-elevated px-4 py-2">
