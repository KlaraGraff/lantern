  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        setThinking(false);
        if (interaction.kind === "word") onLookupSuccess?.(interaction);
        // Recovered content is worth showing, but its schema was damaged and
        // it must not become the cached answer for the next lookup.
        if (response.complete === false) {
          setError(t("learningCard.modelOffFormat"));
          setPartial(true);
          return;
        }
