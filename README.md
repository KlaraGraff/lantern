...
1. **它拿得到上下文**——你的英语水平、读到哪、在读哪一本哪一章、选中处前后是什么，以及它从你过去的提问里总结出的「你想要什么样的解释」，都随每一次请求过去。
2. **它说的每一句都能点回原文**——给的不是一个孤零零的答案，是能带你回到书里那一句的答案。
3. **你读过的，它自己在记**——哪个词你已经不查了、哪个词你又栽了一次、这本书你能认识其中多少词，都从你读书这件事本身长出来，不用你打卡。
...
| 你想要什么样的解释 | 从你过去查的词和追问里自己总结，你也可以直接写给它 |
...
Lantern 把英语水平当作**每一次请求的条件**，而不是界面上的一个偏好。你可以直接填 CEFR 等级（A1–C2），也可以填雅思、托福、托业、剑桥英语、DET 或四六级成绩，由应用换算。讲解语言另有一档单独的选择——中文、中英对照、全英文——推荐值跟着等级走，但选哪一档你说了算，每一档都配了同词同句的样例预览。

但「照顾水平」很容易滑向另一个毛病：为了简单而牺牲准确。所以还有一条硬规矩——**解释里如果非用一个超出你水平的词不可，那就保留它，并且当场就地加注**，用几个更简单的英文词或一个中文括注说清楚。**不许换成更含糊的词，也不许扔在那儿让你另外去查。**

语言难度之外，还有一个独立的**讲解风格**开关，管的是一张卡片给你几条要点：**详解**多给几条——这里是什么意思、怎么认出来、常跟什么走、和相近词差在哪；**简答**只留当场用得上的。两者可以在设置里展开一份同词同句的双栏对比，看清差在哪再选。

<!-- 样张占位：学习偏好设置，以及同一个词在两档水平下的查词卡对比
![按英语水平调整的解释](assets/screenshots/level.png)
-->

### 它知道你想要什么样的解释

水平只说明你能读懂什么，说明不了你想要什么。同样是 B2，有人要看词源，有人嫌词源啰嗦；有人希望例句都从正在读的这本书里取，有人无所谓。

所以还有一页**用户画像**，AI 回答你之前会先读它。上半段是**你自己写的**——喜好、忌讳、学习背景，想说什么写什么。下半段是**系统总结的卡片**，从你最近查的词和追问里归纳，分七个维度：词义讲解、句法讲解、指代讲解、文化背景、查词取向、举例来源、回答节奏。

关键在于这几条边界：

- **每张卡都能点开看依据**——它凭哪几次记录得出这个结论，摊开给你看，不是一句无从核对的判断。
- **冲突时以你写的为准。** 不同意某张卡，可以「移动到上段」改写成你自己的说法，改完这一维度就由你接管，总结器不再动它。
- **可以整份关掉。** 关掉之后 AI 不再读这一页，你写的和已有的卡片都留着，重新打开就继续生效。

<!-- 样张占位：用户画像页——你写的那段、七个维度的卡片、点开的依据
...
- **引用是内联的。** 回答正文里带角标，末尾再列成一排，两处都能点。一次生词清单引用三十几处也不会乱。答案里举的例句同样是链接，点了落在书里那一句上。
- **定位是按原文找的，不是按页码猜的。** EPUB 拿原文片段在对应章节里精确搜索，找不到就换备用片段，再不行退到章节开头；PDF 跳到对应页；纯文本按字符偏移定位。
- **引用可以跨书。** 追问时引到另一本书里的句子，点过去会直接打开那本书的那一句，顶部留一条「从《X》的追问跳转到这里 / 回到《X》」，随时能原路返回。
...
检索原文有一个不显眼但很致命的坑：你问「达西后来怎么了」，书里从头到尾写的是 Darcy——按字面检索，一句都找不到。

所以建索引时 AI 会顺带认出书里的人物，把译名、简称、外号、「那个牧师」这类叫法都归到书里真正的写法上，存成这本书自己的**人物别名表**。表可以手动增删、清空、重建。

同一个称呼指向不止一个人的时候，它不闷头猜：答复上方会明说这次按谁理解的，点一下就能换成另一个人，换完这本书就记住了。**没认出来的时候它也直说**——「没认出『某某』是谁，按原文找的」，然后让你当场指认，指认完立刻生效，随时可撤销。

### 隔了几天回来，存下来的东西还挂在书上

聊天软件里的历史记录是一条越堆越长的流水：你三天前问的那个句子，混在报销流程和菜谱中间，翻回去找不到，找到了也想不起来当时读到哪。生词本是同一个毛病——词孤零零躺在列表里，和它出现的那一句断了联系，剩下的只能硬背。

Lantern 里存下来的东西都还连着书。**笔记**（书签也是笔记的一种，「记住这里」直接就是一条）、**单词**、**问答**各自按书归类，可以搜，每一条旁边都挂着「跳回原文」，点下去落回的不是书的开头，是当时那一句。生词卡上还留着它出现的那段原文，和它**在那一句里**的意思——不是词典里的第一个义项；想看当初存下来时的完整卡片，展开「这个词的完整卡片」，整张都在。

**留痕不用你动手。** 你查过的词、AI 回答里引用过的段落，会自动在正文上留一个轻标记，读到时可以「留下」或「不再显示」。读熟了的词，标记会自己一点点淡掉——注意力该撤走的地方就撤走（不喜欢的话，可以设成永远保持同一深度）。

选中一段话点「解读」，结果也不再是看完就没。**每一次解读都会自动留一份缓存**：下次在同一本书里选中同一段，立刻出结果，不再花一次 API 调用。觉得值得留着，再按一下保存，它才进入你的问答记录——缓存是省钱的，列表是你挑的，两件事分开；忘了按也没损失，回去重选一次就还在。

笔记不必离开正文去找：页边笔记面板把笔记放在它所引用的段落旁边，点一条笔记，原文与笔记仍在同一个视野里。对话更进一步——书打开的同时，**那段对话也跟着回到侧栏**，你接着上次的话头继续问，里面的角标依然是活的。
...
一个词出现在你读过的那一屏上，而你没有查它——这就是「你认识它」的证据。Lantern 把它记下来，够了就自动升一档：**新词 → 学习中 → 眼熟 → 读顺了**。
...
真正开始练时，原句不会被丢掉。Lantern 把生词从原句里挖空，第一层提示是读音，第二层是你已保存的中文句意，最后才揭开答案——不拿字母数和首字母替你猜。间隔安排走官方 FSRS 库。

读完一章，正文末尾会多出**一行小字**：「这一章你查了 N 个词。趁着还有印象，过一遍 →」。就一行，不喜欢可以永久关掉。

### 这本书对你难不难，本地就算得出来

「这本书是 B2 级」是出版社封底干的事，而且不准。Lantern 拆成两个都能回答的问题，两个都在本机算，**不联网、不消耗任何 AI 额度**。

**这本书本身有多重。** 导入时把全文的每个词映射到一张五万词的词频表上，得到一条频段分布——多少词落在最常用的一千个里，多少落在五千名以后。这跟谁来读无关，是书的属性。

**你认识其中多少。** 这一条才是「对你」：拿你已经认识的词去数正文，每一百个词能盖住多少个。结论不是一个孤立的百分数，而是放在一把尺子上——尺子上有两条参考线（95% 和 98%），取自应用语言学里常用的两个阅读理解阈值（Nation, 2006），落在哪一段对应「现在读查词会很密 / 读得下来会常查词 / 可以自己读下去」。展开还能看到这本书的词次构成（读顺了 / 眼熟 / 人名地名 / 还不认识），以及**「哪些词认下来提升最大」**——出现四十次以上的那几个词认下来，覆盖率会从多少升到多少，一句话说清。

三条自划的边界：

- **记录不够就给区间，不给假精度。** 你刚开始用，它会明说「现在只能给一段范围」，并列出这个范围是从哪几本书、多少条记录推出来的；范围横跨了参考线时，它宁可说「现在还分不出来」。
- **永远不会自己改动你的水平设置。** 证据再强也只提醒，不代劳。
- **书太短、或者这个格式取不出正文，就直说不下结论**，而不是硬给一个数字。

第一次打开一本书之前，这些会先汇成一张**开书卡**：这本书的词汇构成、最吃力的一段大概在哪几章、按你最近的阅读速度大约要读多久。**只在首次打开出现这一次**，之后想再看去书籍详情。扫描件会在这里直接给出「下载并识别」——识别在本机进行，不上传原文件，而且可以先读起来，识别完划词和 AI 会自动可用。

<!-- 样张占位：书籍详情页的「这本书对你」——覆盖率标尺 + 词次构成 + 哪些词认下来提升最大
...
判断你的真实水平时，还有一层甄别：一本书自己的题材词汇（航海小说里的帆缆、烹饪书里的火候）会被排除在对照之外，否则一本书的用词偏好就能把结论带偏。这层甄别默认交给 AI（只发少量生词和书名），也可以改成只用本机记录、完全不联网。
...
Lantern 内置了一个 **MCP 服务器**。打开它，Claude Code、Codex 这些 AI 客户端就能直接读你的阅读数据——**你不用复制粘贴任何东西**：书目、书单、元数据与索引状态；你的 CEFR 等级和雅思 / 托福等成绩（**和内置 AI 用的是同一份**）；生词本、掌握状态、统计、查询历史、词形标记；高亮、书签、笔记、书内对话历史、全书与章节摘要。

一共 29 个工具：12 个只读上下文查询、一个「在阅读器中打开」动作、16 个写入工具。默认只读；**写权限（导入书、删书、建书单这些）是一个单独的开关，默认关着**——把数据交出去和把控制权交出去是两回事，不该捆在一起。删数据或覆盖式导入还会针对那一次操作单独确认；MCP 不会替你的 AI 再去调用模型。
...
**默认值会拦你一下。** 应用自己也在正文上画东西：朗读位置是一层冷蓝洗色，生词 / 学习中 / 读顺了是三档下划线，从暖橙到青到灰色虚线——那是一条「注意力逐步撤走」的渐变。你挑的颜色如果和它们太接近，设置里会当场提示「正文里可能分不清」，并指出撞的是哪一个。这个判断是把两个颜色**在真实纸底上混合之后**再算色距，不是比 HEX——一个洗色和一条下划线在色板上离得远，铺到纸上可能就分不开了。
...
排版走的是同一套逻辑，而且**默认跟着出版方的意图走**：中西文字体分开设置，英文用英文字体、中文用中文字体，不互相将就；首行缩进和段间距互斥——开了缩进，段间距一律压平（引文块和空段落除外），因为同时用两套分段信号是排版上的重复。先有一套全局默认，一本书可以保留自己的例外；设置面板会明确告诉你当前是「本书单独设置」，并允许你恢复全局、把这本书的效果提升成新的全局默认，或者再挑几本书一起应用——不为了管理作用域再叠一个弹窗。
...
2. **填一个 AI 服务** —— 「设置 → AI 配置」，填 OpenAI 兼容 API、Anthropic、Ollama 或自建网关，测试连接。密钥只存在这台设备上。
3. **拖一本书进来** —— 把 EPUB / PDF / TXT 拖进书库，打开它，**双击任意一个单词**。

想让它更贴合自己：去「设置 → 学习偏好」填英语等级和讲解语言，去「设置 → 个人」写两句你想让 AI 先知道的事，去「设置 → 划词与卡片」调你想看的模块。
...
语境查词 · 短语释义 · 段落解读 · 整段翻译 · 书内持续对话 · **每条回答带可点击的原文引用，例句也是链接，点一下跳回书里那一句** · 引用可跨书跳转并原路返回 · 全文索引检索，配了 embedding 再叠一层语义检索 · 回答范围可选自动 / 选区 / 本章 / 全书 · **阅读思考保护**：默认只用你已读过的部分回答，可一键改为结合全书 · **人物别名表**：用中文名、简称、外号都能检索到只写英文名的段落，指代不清时明说按谁理解并可当场换人 · 人物 / 术语轻卡整理身份、关系、书内含义与此前出场，同样默认截至当前位置 · 明确展示本次带进对话的原文，可随时移除 · **问答按书归档、可搜索**，一键跳回原文；解释自动缓存，同一段再选中不花第二次调用

</td></tr>
<tr><td><b>它认识你</b></td><td>

**用户画像**：上半段你自己写，下半段由系统从你的查词与追问里总结成七个维度的卡片（词义讲解 / 句法讲解 / 指代讲解 / 文化背景 / 查词取向 / 举例来源 / 回答节奏）· 每张卡可展开看依据的原始记录 · 不同意可「移动到上段」改写并由你接管 · 冲突时以你写的为准 · 可整份关闭，已有内容保留 · 学习者等级（CEFR，或由雅思 / 托福 / 托业 / 剑桥 / DET / 四六级换算）· 讲解语言：中文 / 中英对照 / 全英文，每档带样例预览 · **讲解风格：详解 / 简答**，附同词同句双栏对比

</td></tr>
<tr><td><b>学习闭环</b></td><td>

生词本 · 查询历史 · **四档掌握度（新词 / 学习中 / 眼熟 / 读顺了），读到不查自动升、又查一次自动退** · 每个词可查「它是怎么走到这一步的」时间线，随时按「我其实不认识」拨回 · 掌握度按词保存、跨书共用 · 生词卡可整张重新生成，也可展开「收藏时的完整卡片」· **复习页按来由分堆**（本书反复查的、以为会了又查的、刚读完那章查过的、很久没再遇到的），不是算法队列 · 官方 FSRS 间隔重复 · 原句挖空复习，读音 → 已保存中文句意 → 答案逐层提示 · 章末一行小字提示回顾，可展开可关 · 笔记中心与页边笔记面板（书签并入笔记）· CSV / JSON 词汇导入导出 · 高亮、生词与备注导出为 Markdown / CSV / Anki CSV

</td></tr>
<tr><td><b>这本书对你</b></td><td>

导入时在本机统计全书词频，映射到五万词词频表的各频段 · **覆盖率：用你已经认识的词去数正文，落在 95% / 98% 两条阅读理解参考线（Nation, 2006）构成的尺子上** · 展开可见词次构成（读顺了 / 眼熟 / 人名地名 / 还不认识）与「哪些词认下来提升最大」· 记录不足时给区间并列出依据，横跨参考线时明说分不出来 · 可选是否把「眼熟」算作认识、是否在书架卡片上显示百分比 · **开书卡**：首次打开一本书前说明词汇构成、最吃力的章节、按你的速度大约要读多久，只出现一次 · 水平对照会排除本书的题材词汇 · **不联网、不消耗 AI 额度、永不自动改动你的水平设置** · 文件过短或格式取不出正文时明确不下结论

</td></tr>
<tr><td><b>正文标记</b></td><td>

**查过的词与 AI 引用过的段落自动留痕**，读到时可留下或不再显示，读熟后标记自行淡出（可关）· 三档匹配范围：当前位置 / 全书同词 / **同词各词形**（AI 一键生成或批量补全，可手动改）· 已保存生词可显示词上小字或页边释义，无需再次调用 AI · 手动高亮 · 手动与自动两套独立样式，各含颜色、透明度、高亮 / 下划线 / 加粗与字体 · 选色撞上系统标记时当场警告 · 不改动原书内容

</td></tr>
<tr><td><b>阅读体验</b></td><td>

暖色纸感主题与自定义背景色 · 深色跟应用内选的那一档，不问操作系统 · **中西文字体分开设置** · 自定义字体导入与设备本地增强字体包 · 字号、行距、字间距、词间距、页边距、两端对齐、语言断词 · **首行缩进与段间距互斥，默认跟随出版方意图** · 全局默认与本书单独设置，可恢复或提升为全局 · 滚动与分页切换 · 目录侧栏 · 阅读进度 · 页边笔记面板 · 多窗口同时读多本，新窗口层叠错开 · 书单管理书库 · 导入时读取 EPUB 自带的书名与作者 · 读完一本时就地标记已读完
...
<tr><td><b>AI 服务与索引</b></td><td>

OpenAI 兼容 API · Anthropic · Ollama · 可选 OpenAI OAuth · 可加多个服务并设优先级 · 每个服务可存多个 Key，输出开始前按优先级自动试可用的 Key 和服务 · 连接测试与模型发现 · **为整个书库批量建索引**，可停可续 · 单本书的索引分切分块 / 定位句 / 生成向量 / 章节摘要 / 人物姓名五个阶段，失败可断点重试，另有重新切分块与重写章节摘要

</td></tr>
<tr><td><b>集成与工具</b></td><td>

MCP 服务器——把书库、生词本和笔记开放给 Claude Code、Codex 等 AI 客户端，写权限独立开关且默认关闭 · 扫描版 PDF 的 OCR，可边识别边读 · 可编辑的书籍来源站点列表 · 书籍元数据编辑 · 应用内检查并安装更新
...
- **书库数据本地优先。** 书、阅读进度、生词本、笔记、用户画像和阅读统计都存在这台设备上。
