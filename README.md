...
- **默认不剧透。** 「阅读思考保护」让它只用你已经读过的部分来回答，答案上会标明「已按你的阅读进度回答（前 X%）」。想要完整答案，点一下「结合全书重新回答」就行。这个开关可以按书单独设。
...
语境查词 · 短语释义 · 段落解读 · 整段翻译 · 书内持续对话（把单词、原句、已有释义和阅读位置一并带进对话）· **每条回答带可点击的原文引用，点一下跳回书里那一句** · 全文索引检索，配了 embedding 再叠一层语义检索 · 回答范围可选自动 / 选区 / 本章 / 全书 · **阅读思考保护**：默认只用你已读过的部分回答，不剧透，可一键改为结合全书 · 明确展示本次带进对话的原文，可随时移除
