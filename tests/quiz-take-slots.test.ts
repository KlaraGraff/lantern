...
import type { Quiz, QuizConfig, QuizGenerationPlan } from "../src/quiz/types.ts";
...
describe("deriveSlots", () => {
  ...
  it("前篇 done 即 open；后篇在生成中是 pending", () => {
    const plan: QuizGenerationPlan = {
      groups: [
        { words: ["alpha"], state: "done", passageId: "p1" },
        { words: ["beta", "gamma"], state: "pending" },
      ],
    };
    const session = makeSession({ articles: [{ wordCount: 1, step: "ok" }, { wordCount: 2, step: "writing" }] });
  ...
  it("锁的是做题顺序：第 2 篇先生成好，第 1 篇没好时它是 locked 不是 open", () => {
    const plan: QuizGenerationPlan = {
      groups: [
        { words: ["alpha"], state: "pending" },
        { words: ["beta"], state: "done", passageId: "p2" },
      ],
    };
    const session = makeSession({ articles: [{ wordCount: 1, step: "writing" }, { wordCount: 1, step: "ok" }] });
    const slots = deriveSlots(makeQuiz("generating", plan), session);
  ...
  it("会话在跑但该篇已终局失败 → failed；其后的 done 篇仍是 locked", () => {
    const plan: QuizGenerationPlan = {
      groups: [
        { words: ["alpha"], state: "failed", errorCode: "AI_STREAM_FAILED" },
        { words: ["beta"], state: "done", passageId: "p2" },
      ],
    };
    const session = makeSession({ articles: [{ wordCount: 1, step: "failed" }, { wordCount: 1, step: "ok" }] });
    const slots = deriveSlots(makeQuiz("generating", plan), session);
  ...
  it("重启冷启动（无会话）：未完成篇一律 failed，不管计划里记的是 pending 还是 failed", () => {
    const plan: QuizGenerationPlan = {
      groups: [
        { words: ["alpha"], state: "done", passageId: "p1" },
        { words: ["beta"], state: "pending" },
        { words: ["gamma"], state: "failed" },
      ],
    };
  ...
  it("done 但 passageId 在卷面里找不到（数据异常）按未完成处理，不产出空 open 篇", () => {
    const plan: QuizGenerationPlan = {
      groups: [{ words: ["alpha"], state: "done", passageId: "ghost" }],
    };
    const slots = deriveSlots(makeQuiz("generating", plan), undefined);
