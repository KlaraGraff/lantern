    ...
    async load(src, afterLoad, beforeRender) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const finish = doc => {
                ...
                // A loaded frame whose document is inaccessible or empty (the
                // embedder's CSP/origin rules refused the navigation and left
                // a substitute document) can never render. Fail fast with a
                // clear code rather than stranding this promise until the
                // consumer's outer timeout.
                if (!doc?.body) {
                    reject(new Error('IFRAME_DOCUMENT_INACCESSIBLE'))
                    return
                }
                try {
                    afterLoad?.(doc)
                    ...
                    // Nothing above may strand the promise: surface render
                    // failures to the caller instead of hanging forever.
                    reject(error)
                }
            }
            const onLoad = () => {
            ...
            // the whole reader (surfaced as READER_INIT_TIMEOUT). The document
            // itself is parsed well before its sub-resources settle, so fall
            // back to rendering the already-parsed DOM after a grace period —
            // the resize observer above still repaints once slow resources
            // eventually load. If even parsing never produced a <body>, leave
