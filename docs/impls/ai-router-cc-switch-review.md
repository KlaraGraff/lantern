...
> **实施后修订（2026-08-05）：** 这一条的严重性在调研时被低估了——真实场景比「并发」更糟，见 [6.1](#61-35-故障转移切换去重已落地)。已落地。
...
### 6.1 3.5 故障转移切换去重（已落地）

提交 `5ebb09c`。这一条在第 3 节里给的是「可做，低优先级」，但没有进第 5 节的切割清单——是遗漏，补做时发现它比调研时写的严重。
...
**一个刻意的取舍。** 表是进程级的、只增不删。profile 对数是个位数，一条记录几十字节，加清理逻辑的代码比它省下的内存贵。
