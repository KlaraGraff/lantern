# 出卷页图片取词（今天的词 · 粘贴截图 → AI 提词）

样张：`quiz-image-words-mockup.html`（已拍板，五状态 A–E）。本文是代码级实现计划。

> **实现期修订：v1 砍掉拖入（样张 B 不落地）。** 本应用的拖放被 Rust 原生接管
> （tauri `dragDropEnabled`，见 Home.tsx 的拖入导入注释：webview command 永远
> 不接收任意绝对路径，这是安全边界），HTML5 drop 事件在窗口内根本不触发；
> 要接就得加一条「按路径读文件」的 command，破这条边界。粘贴 + 选文件按钮
> 已覆盖主流程，拖入真有需求再单独立项过安全设计。

## 产品口径（已拍板）

- 入口~~三个~~**两个（v1，见上）**：**粘贴**（Cmd+V）、右下角小按钮**「从图片提取」**选文件。落图即自动开始识别，不要确认步骤。
