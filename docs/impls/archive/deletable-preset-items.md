...
> **状态：已实施并落地（2026-08-03 核对）。** 数据层在 `2e8127d`，UI 与阅读中空态在 `9159a9c`，两个提交都已在 `main` 上。**块 1–7 全部完成，块 8 只剩索引与归档。**
>
> ⚠️ 本文档此前的状态行写着「待实施，不含任何已落地的代码」——那是文档提交（`d09b3d5`）与实现提交之间的一天时差留下的，**已作废**。接手时以代码为准。
>
> **一处规范偏离，已修复：** §3.2 第三行要求「自定义项 / 列表还有内容 / 需要确认」，但 `BookSourcesSettings.tsx` 把 `presetDeleteConfirm` 的第一个参数写死成 `"builtin"`，导致用户自己添加的书源在列表非空时**静默删除且不可找回**（`restoreBuiltInBookSources` 只保留仍在列表里的用户条目）。现已改为按 `bookSourceDeleteKind(source.id)` 判定，并由 `tests/book-sources.test.ts` 锁住。
>
> **仍需人工点界面验收：** 12 条，见文末[附录](#附录人工验收清单2026-08-03-核对结论)。代码能证明的部分已全部证明（330 前端单测 / 708 后端测试 / tsc / eslint / clippy 全绿），剩下的是重启后是否复活、空态视觉、键盘可达性这类只能用眼睛验的。
>
...
## 附录：人工验收清单（2026-08-03 核对结论）

代码层面的核对已经做完：块 1–7 全部落地，S1–S4 四种墓碑状态（含最容易做错的「S4 未来版本新增的内置项仍应出现」）都实现正确并有测试锁住，墓碑解析带白名单/去重/上限，三个 kind 不串台。下面这些是**读码证明不了、必须有人点一遍**的，按顺序走即可。

1. 设置 → 阅读辅助 → 操作菜单 → 单词：删掉「复制」，**完全退出应用再启动**，确认它没回来。
2. 同一节删到只剩 1 项再删：弹框标题应是「删除最后一项？」且正文是菜单版文案；点「取消」后条目还在。
3. 删光后目视：tab、小标题、说明文字、「添加自定义动作」、「恢复默认」都还在，空态那一行不是塌成 0 高的双层边框。
4. 点「恢复默认」：7 个内置动作按出厂顺序回来、`translate` 是关的、之前加的自定义动作还在且排在内置项之后。
5. 卡片设计 → 单词：删光全部模块后，密度／卡片宽度／例句数量三行仍在且可改；右侧预览显示空态文案而不是白屏；「已启用 N 项」显示 0。
6. 阅读界面：卡片模块删光后划词点「查词」或按快捷键 → 只弹 toast、不弹卡片、控制台无报错。
7. 阅读界面：操作菜单删光后划词 → 什么都不弹、没有空边框浮层；此时**双击查词照常开卡**（它绕过菜单）。
8. 阅读界面：菜单只剩「朗读」时，菜单正常打开且只有那一行。
9. 键盘：在三处列表里用 Tab 走到删除按钮，确认按钮此时**可见**（`focus-visible:opacity-100` 已写，需目视确认没被父层 `overflow` / `opacity` 盖住）。
10. 三个 kind 互不串台：删单词卡／单词菜单的条目后，切到短语、段落 tab 看是否原样。
11. 书籍来源：删到全空再删 → 弹确认；确认后重启仍全空（哨兵键不重播种）；只清空「图书馆」分组而「第三方资源站」还有条目时**不**弹确认；**删自己添加的书源时必须弹确认**（本次修复项）。
12. §2.2 表格里的四处列表各自端到端走一遍。

### 一条产品上要知道的行为（不是缺陷）

某个 kind 被删光后，将来版本若往 `MODULE_DEFINITIONS` 里新增内置模块，该模块会被补进这份空列表，卡片于是重新开始弹出。这正是 §4.1 要的 S4 语义（新增的内置项应该出现），与「删光 = 永久关闭」不可兼得。用户想永久关闭，应该用开关而不是删除。

### 两处已知的低风险遗留

- `Reader.tsx` 调 `readerMenuRows` 时没传 `showTranslate`，而 `ReaderContextMenu.tsx` 传了。今天不咬人——全仓库没有任何调用方真的传入该选项，两侧都取默认 `false`。但这正是 §9.2 点名的陷阱：一旦有人重新接上 `showTranslate`，「菜单被删光但 translate 被注入」时守卫会算 0，菜单静默不弹，而它本该渲染一行。
- `ToolsSettings.tsx` 里 `show_translation` 的派生逻辑（模块被删 → 写 `false`）正确，但没有测试覆盖。§9.5 点名这是「唯一会真正咬人」的耦合，目前靠读码保。
