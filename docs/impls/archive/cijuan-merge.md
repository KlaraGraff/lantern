...
> C=旧数据不迁移；D=演示模式只留测试。样张 v2 已验收（2026-08-12，六条附带决议全按推荐，
> 见 §四.9–14）；解析为两阶段生成（§二.6）。当前：步骤 2/3 已完成并落 main
> （后端 8d18986、TS 136406c，2026-08-13，双审查 PASS）；下一步：步骤 4 UI 实现。
...
二审补两条（2026-08-13）：① 入参加 `request_id`，接入现有 `ai_cancel` 取消通道——
样张「正在出卷」屏有取消按钮，没有句柄的话点取消后所有分组请求照跑照计费；② 请求
purpose 用 `Analysis` 而非 `Utility`——Utility 会把 thinking 钉死为关闭，而明答校验的
本职就是推理解题，出题正确性不在这里省。
...
- **缓存链**：出题 → 明答校验（续写，付缓存写入）→ 解析生成（续写，命中 ~0.1 倍读取价），
  ...
  流程不受影响。
  实现细节（2026-08-12 审查后细化）：Anthropic 缓存要求前缀**逐字节一致**，所以出题调用
  实际发送的 user 消息（含 schema 附块）与模型原始回复要在内存里留一份「续写凭据」（trace，
  不持久化），明答校验与阶段二都以它做前缀续写；冷启动补生成兜底本就超出缓存时效，照旧
  重建提示词。后端 `cache=true` 在最后一条消息与前缀里最后一条 assistant 消息两处打
  cache_control 断点——解析生成与明答校验共享前两条消息但末条不同（分叉续写），必须在
  assistant 消息处有独立断点才能命中共享前缀。
  勘误（2026-08-13 二审）：明答校验的续写**不能**原样回放阶段一回复——那里面带着每题的
  `answer` 字段，「独立重解」的模型抬眼就能看到答案键，校验形同虚设。缓存写发生在阶段一
  请求本身，缓存前缀只覆盖到那条 user 消息，assistant 轮本就不在前缀里、不要求字节一致，
  所以校验的 assistant 轮改用**去掉答案字段**的阶段一回复重新序列化——缓存读照常命中，
  答案键不进上下文。阶段二解析生成不受此限（它本来就该看到答案），仍原样回放。
...
| 1 | 静态样张：词卷区在 Lantern 视觉体系下的全部关键屏（入口/出卷设置含生词本导入/生成中/做题/评卷/错词池/历史/追问抽屉，含空态、失败态、交卷确认） | 用户逐屏认可（样张一次全给，不分批） | ✅ v2 已验收（2026-08-12） |
...
样张 v2 验收决议（2026-08-12，六条全按推荐拍板）：

9. **回顾板原有堆卡入口**同样改跳翻卡独立页——一个动作只有一个去处，不维护两套翻卡容器
   （属对现有复习 UI 的范围新增，随步骤 4 实现）。
10. **词卷页直接落在「出卷」标签**，不设总览页——出卷是最高频动作，总览多一跳。
11. **词卷入口按钮不带数字**——错词池计数不是待办，挂在入口有催办感，违背「复习不带压力」
    的文案原则；翻卡入口保留待复习数。
12. **不设「一次性补全所有讲解」批量按钮**，仅单题补生成——多题缺讲解只在阶段二整体失败时
    出现，罕见兜底态不配常驻 UI。
13. **语法卷标题与阅读卷统一衬线**——体裁区分靠题型标签已足够。
14. **页面承载关系照样张**：出卷/错词池/往卷为词卷页内标签；做题/评卷整页接管、导航换「退出」。
