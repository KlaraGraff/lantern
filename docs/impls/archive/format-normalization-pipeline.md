...
- [x] **Layer B：给 foliate paginator 的 iframe `load` promise 加超时 fallback。** `paginator.js` 的 `load()` 现在除了监听 iframe `load` 事件，还起一个 15s `IFRAME_LOAD_TIMEOUT`：超时后若文档已解析出 `<body>` 就直接渲染已解析的 DOM（子资源晚到由既有 ResizeObserver 重绘），双跑用 `settled` 标志防护。这样卡死的子资源不再让 `view.init()` 永久挂起。
  ...
  - **⚠️ 分叉代价（接手者注意）：** 现已脱离上游 `yicheng47/foliate-js`。上游后续的 PDF 修复需手动 cherry-pick / merge。理想情况将 Layer B 也向 `yicheng47` 提 PR，合并后可考虑改回上游实现。

> **仓库整合（2026-07-17）：** 已将 `public/foliate-js` 从 `KlaraGraff/foliate-js` 子模块转换为 Lantern 内受版本控制的源码目录，保留当时的 `112eb27` 内容和既有修复。上述子模块/fork 记录是历史背景；后续引擎更新直接提交到 Lantern。
...
- **2026-07-17** — 真机确认 EPUB/PDF/转换书全部可打开。原 v2.0.0 因无法打开书籍而撤回；修复后的版本以 **v2.0.0** 重新发布（含 CSP 根因修复 + 转换管线 + Layer A/B + pdfium 授权修复）。交接测试文档中被阅读器故障阻塞的 T4–T8/T10 现已解除阻塞，可继续验收。
