...
> **后记（2026-08-14，实测后修订）**：下文 §一.3 的「按篇序解锁」已废止，改为
> **谁就绪谁开放**——某篇生成完就能做，不看前面的篇。实测撞到的反例：第 1 篇生成
> 失败、第 2 篇已就绪，屏幕显示「1/2 篇就绪」却一篇都做不了，用时还在走。篇号仍
> 等于拆词组下标（页签位置稳定），只是中间没就绪的篇原地显示占位面板。槽位状态
> `locked` 与它的文案随之删除；做题页在没有任何可做篇时不计时、不显示用时。
> 同批还给单篇生成加了自动重试（`QUIZ_ARTICLE_MAX_ATTEMPTS`，见 `src/quiz/generate.ts`）：
> 失败先自动重试，用尽次数才落到 §一.5 的「未生成完成 + 单篇重新生成」；设置类
> 错误码不重试。§一 末尾那条「遗留的产品问题」（放弃某篇按短卷交卷）已拍板
> **不做**（issue #16）：维持现状——反复失败的卷留在往卷里，换个模型或等服务
> 恢复后单篇重新生成。依据见下一段实测：那是会自己好的瞬时故障，不是某一篇
> 永远生不出来。
>
> **实测到的失败形态（决定了退避时长）**：线上第一次撞到的整篇失败，是请求在
> 约 0.7s 被掐断（应用日志 `ai router: ... failed kind=network`，没有 HTTP 状态
> 码，因此前端拿不到任何已知错误码，界面只能说「未生成完成」）。它成片出现：
> 一分钟内连着九次全失败，两分钟前后同一 profile 又正常出稿。所以退避铺到约
> 一分钟（3s → 15s → 45s）才有意义，几秒钟的重试只是把同一秒的故障重放几遍。
> 同批把失败原文一路带到界面（`errorMessage`）、把 `detail=` 加进 `router.rs`
> 的失败日志——上一轮正是因为两处都没有，才无从判断这一篇为什么失败。
...
**遗留的产品问题 —— 2026-08-14 已拍板：不做**。某篇反复重生成失败时，不给「放弃这篇、按短卷交卷」的出口；卷子留在往卷里，换模型或等服务恢复后单篇重新生成。当初的判断（失败多为瞬时或设置类）被实测证实：第一次真撞到的整篇失败是服务端成片抽风，两分钟后同一 profile 又正常出稿。issue #16 据此关闭。
