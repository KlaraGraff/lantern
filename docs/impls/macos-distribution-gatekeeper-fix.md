...
## 5.5 已评估并否决的替代方案(2026-07-17,记录避免重复讨论)

- **"像原来一样显示不安全、让用户手动授权"** — 不可行。旧式"无法验证开发者 → 右键打开/系统设置里'仍要打开'"的旁路属于旧版 macOS;macOS 15 起 Apple 移除了右键打开旁路,且对 **ad-hoc 签名 + quarantine** 的应用直接报"已损坏"、只给"移到废纸篓",不提供任何授权入口(本机 macOS 26.5 实测确认)。这不是我们能在打包侧恢复的行为;Apple Silicon 上二进制又必须至少 ad-hoc 签名(无法"彻底不签"退回旧提示)。唯一的用户侧出口就是 §3 的 `xattr`,即方案 B。
- **"本地打包后上传产物、不用 GitHub 打包"** — 无效。"已损坏"的触发条件是 **ad-hoc 签名 + 下载时的 quarantine 属性**,与在哪台机器构建无关:本地包同样是 ad-hoc(`signingIdentity: "-"`),任何人(包括自己换台设备)从 GitHub 下载后一样带 quarantine → 一样"已损坏"。本地构建只对"构建机本机直接安装、不经下载"这一种场景有效。结论:**上传本地包解决不了分发,只有签名(方案 A)能让下载即用。**
...
- **2026-07-17(续)** — 评估并否决两个替代方案(旧式手动授权、本地打包上传),分析记入 §5.5。惯例固化:`AGENTS.md` 新增 **Release Conventions**(禁止复用已发布版本号 / 以关于页 commit 识别构建 / 发布后验收实物产物);release skill 增加版本号防复用守卫与发布后产物验收步骤(第 10 步)。更正 §6:构建指纹展示早已存在于 设置→关于。
