...
- 先修复当前 `source_file_path` 在导入事件与同步验证之间的路径契约不一致。
...
这很可能让合法的原生书籍导入事件报 `SYNC_BLOB_PATH_INVALID`。在引入 OCR 资产前，应统一“源资产可能位于 `books/` 或 `sources/`”的角色规则，并让 event、snapshot 和路径 resolver 使用同一套白名单验证；同时补充双设备 replay 回归测试。该问题独立于 OCR，却直接影响新资产同步的可信基线。
