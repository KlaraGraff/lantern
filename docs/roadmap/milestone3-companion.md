...
- **Lantern remains local-first** — all data stays in local SQLite + iCloud sync. Backend is an optional upgrade, not a dependency
- **Two backend endpoint types:**
  - AI endpoints — drop-in replacement for direct LLM calls, returns personalized responses
  - Signal collection — Lantern pushes reading signals (highlights, notes, vocab, chats) for persona model updates
