import type { Quiz, QuizConfig, QuizWord } from '../types'
import type { GenerateStep, ProgressFn } from './generate'

/**
 * 演示模式：不调 API，用内置样卷把「出题 → 作答 → 判分 → 错词入池」整条链路跑通。
 * 用途：没有 key 时体验产品、本地开发、自动化测试。
 */
...
export async function generateMockQuiz(opts: {
  ...
  const steps: GenerateStep[] = opts.config.maskedCheck
    ? ['splitting', 'writing', 'maskedCheck', 'done']
    : ['splitting', 'writing', 'done']
