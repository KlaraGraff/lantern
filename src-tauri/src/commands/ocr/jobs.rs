...
fn row_to_job(row: &rusqlite::Row<'_>) -> rusqlite::Result<OcrJob> {
    Ok(OcrJob {
        ...
        recognized_pages: row.get("recognized_pages")?,
        skipped_pages: row.get("skipped_pages")?,
        timed_out_pages: row.get("timed_out_pages")?,
        failed_pages: row.get("failed_pages")?,
        temporary_path: row.get("temporary_path")?,
