...
use super::prompt::{
    book_reference_block, explanation_strategy, normalized_explanation_mode,
...
fn explain_system_prompt(explanation_mode: &str, cefr: &str, explanation_style: &str) -> String {
    format!(
        ...
        explanation_strategy(explanation_mode, cefr, explanation_style),
    )
}
...
pub async fn ai_explain(
    ...
    let (explanation_mode, cefr, explanation_style) = {
        let conn = db.reader();
    ...
    let mut system_prompt = explain_system_prompt(&explanation_mode, &cefr, &explanation_style);
...
mod tests {
    ...
    fn explain_prompt_asks_for_brevity_and_no_headers() {
        let p = explain_system_prompt("english_by_level", "B1", "thorough");
    ...
    fn explain_prompt_english_emits_english_directive() {
        let p = explain_system_prompt("english_by_level", "A2", "thorough");
        assert!(p.contains("Write explanations in English at CEFR A2."));
    }

    #[test]
    fn chinese_mode_explains_in_chinese() {
        let prompt = explain_system_prompt("chinese", "B2", "thorough");
    ...
    fn explain_prompt_never_has_translation_gloss() {
        ...
        for mode in ["english_by_level", "chinese", "adaptive_bilingual"] {
            let p = explain_system_prompt(mode, "B1", "thorough");
            assert!(
